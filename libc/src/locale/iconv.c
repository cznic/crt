...
//TODO(ccgo)		unsigned nel = sizeof rev_jis / sizeof *rev_jis;
	unsigned d, j, i, b = 0;
	for (;;) {
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)			i = nel/2;
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)			j = rev_jis[b+i];
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)			d = jis0208[j/256][j%256];
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)			if (d==c) return j + 0x2121;
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)			else if (nel == 1) return 0;
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)			else if (c < d)
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)				nel /= 2;
		__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)			else {
...
//TODO(ccgo)					c = jis0208[c][d];
		...
		case GB18030:
			...
			if (d-0x40>=191 || d==127) {
				...
				c += 128;
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)					for (d=0; d<=c; ) {
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						k = 0;
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						for (int i=0; i<126; i++)
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							for (int j=0; j<190; j++)
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)								if (gb18030[i][j]-d <= c-d)
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)									k++;
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						d = c+1;
...
//TODO(ccgo)				c = gb18030[c][d];
		...
		case BIG5:
			...
			if (c-0xa1>=0xfa-0xa1) {
				...
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)					c = (hkscs[4867+(c*157+d)/16]>>(c*157+d)%16)%2<<17
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						| hkscs[c*157+d];
				...
				if (c/256 == 0xdc) {
					...
					char *ptmp = tmp.c;
					__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						size_t tmpx = iconv(combine_to_from(to, find_charmap("utf8")),
					__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							&(char *){"\303\212\314\204"
					__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							"\303\212\314\214"
					__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							"\303\252\314\204"
					__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							"\303\252\314\214"
					__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							+c%256}, &(size_t){4},
					__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							&ptmp, &(size_t){sizeof tmp});
...
//TODO(ccgo)						if (tmpx) x++;
			...
			c -= 0xa1;
			__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)				c = big5[c][d]|(c==0x27&&(d==0x3a||d==0x3c||d==0x42))<<17;
			if (!c) goto ilseq;
			break;
		case EUC_KR:
			if (c < 128) break;
			...
			if (c >= 93 || d >= 94) {
				...
				c += 0xac00;
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)					for (d=0xac00; d<=c; ) {
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						k = 0;
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						for (int i=0; i<93; i++)
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)							for (int j=0; j<94; j++)
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)								if (ksc[i][j]-d <= c-d)
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)									k++;
				__assert_fail("TODO(ccgo)", __FILE__, __LINE__, __func__);
//TODO(ccgo)						d = c+1;
