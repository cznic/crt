	...
	char *const argv[restrict], char *const envp[restrict])
{
	...
	args.argv = argv;
	args.envp = envp;
	//TODO(ccgo)	pthread_sigmask(SIG_BLOCK, SIGALL_SET, &args.oldmask);
	// cpp:	pthread_sigmask(0, ((sigset_t *)(const unsigned long long [2]){ -1,-1 }), &args.oldmask);
	unsigned long long lit[2] = {-1, -1};
	pthread_sigmask(0, (sigset_t *)&lit, &args.oldmask);
	...
	if (pid > 0) {
		...
		//TODO(ccgo)	else waitpid(pid, &(int){0}, 0);
