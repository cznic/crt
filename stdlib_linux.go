...
// standard.  These functions shall convert the initial portion of the string
...
// character; then an optional exponent part consisting of the character 'e' or
// the character 'E', optionally followed by a '+' or '-' character, and then
...
// consisting of the character 'p' or the character 'P', optionally followed by
// a '+' or '-' character, and then followed by one or more decimal digits
//
// One of INF or INFINITY, ignoring case
//
// One of NAN or NAN(n-char-sequenceopt), ignoring case in the NAN part, where:
//
// n-char-sequence: digit nondigit n-char-sequence digit n-char-sequence
// nondigit
//
// The subject sequence is defined as the longest initial subsequence of the
// input string, starting with the first non-white-space character, that is of
...
// <hyphen-minus>, the sequence shall be interpreted as negated. A character
// sequence INF or INFINITY shall be interpreted as an infinity, if
...
// or NAN(n-char-sequenceopt) shall be interpreted as a quiet NaN, if supported
...
// object pointed to by endptr, provided that endptr is not a null pointer.
//
// If the subject sequence has the hexadecimal form and FLT_RADIX is a power of
// 2, the value resulting from the conversion is correctly rounded.
//
// The radix character is defined in the current locale (category LC_NUMERIC).
...
//
// [ERANGE] The value to be returned would cause overflow or underflow.  These
