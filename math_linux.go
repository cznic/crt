...
func Xldexp(tls TLS, x float64, exp int32) float64 { return math.Ldexp(x, int(exp)) }
...
func Xmodf(tls TLS, x float64, iptr uintptr) float64 {
	int, frac := math.Modf(x)
	*(*float64)(unsafe.Pointer(iptr)) = int
	return frac
}

// double frexp(double x, int *exp);
func Xfrexp(tls TLS, x float64, exp uintptr) float64 {
	f, i := math.Frexp(x)
	*(*int32)(unsafe.Pointer(exp)) = int32(i)
	return f
}
...
func X__isnan(tls TLS, x float64) int32 { return Xisnan(tls, x) }
