...
func Xuname(tls TLS, buf uintptr) int32 {
	...
	if strace {
		...
		fmt.Fprintf(os.Stderr, "\tXsysname %q\n", GoString(buf+unsafe.Offsetof(Sutsname{}.Xsysname)))
		fmt.Fprintf(os.Stderr, "\tXnodename %q\n", GoString(buf+unsafe.Offsetof(Sutsname{}.Xnodename)))
		fmt.Fprintf(os.Stderr, "\tXrelease %q\n", GoString(buf+unsafe.Offsetof(Sutsname{}.Xrelease)))
		fmt.Fprintf(os.Stderr, "\tXversion %q\n", GoString(buf+unsafe.Offsetof(Sutsname{}.Xversion)))
		fmt.Fprintf(os.Stderr, "\tXmachine %q\n", GoString(buf+unsafe.Offsetof(Sutsname{}.Xmachine)))
		fmt.Fprintf(os.Stderr, "\tX__domainname %q\n", GoString(buf+unsafe.Offsetof(Sutsname{}.X__domainname)))
	}
	if err != 0 {
		tls.setErrno(err)
	}
